...
\end{itemize}
\paragraph{Cumulative time view.}
The figure below shows cumulative time gained/lost versus progress for all boats in one plot.
Positive values mean a boat is behind the fleet mean at that point on the course, while negative values mean it is ahead.
\begin{figure}[H]
\centering
\includegraphics{figures/\figurevariant/time-delta-along-route.pdf}
\caption{Cumulative time gained/lost versus progress (all boats).}
\end{figure}
\sectionbreak
