...
\sectionbreak
\ifspeedplots
\section*{Pair summaries (pace and speed)}
\else
\section*{Pair summaries (pace)}
\fi
This section compares selected boats directly with split violins by leg.
For each leg, the left half is the first named boat and the right half is the second.
...
\fi
This section summarizes pace deltas (and speed deltas, if enabled) by boat across all legs. Each boat's distribution of deltas is aggregated across all legs to show where the boat was faster or slower than the baseline at the same places on the course. The same violin-plot interpretation applies: negative centers mean faster-than-baseline, positive centers mean slower-than-baseline, and broad/long shapes flag inconsistency or specific locations with large time loss.
\subsection*{Saga}
\begin{figure}[H]
\centering
