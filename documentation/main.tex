...
\fi
This section summarizes pace deltas (and speed deltas, if enabled) by boat across all legs. Each boat's distribution of deltas is aggregated across all legs to show where the boat was faster or slower than the baseline at the same places on the course. The same box plot anatomy applies: negative centers mean faster-than-baseline, positive centers mean slower-than-baseline, and tall boxes or long whiskers flag inconsistency or specific locations with large time loss.
\subsection*{Saga}
\begin{figure}[H]
\centering
